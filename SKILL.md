...
1. **Build file tree** — Use Glob to list all source files. Exclude these directories:
...
Scan source files for import/require statements to build a dependency graph. Read `references/detection-patterns.md` for the full list of import patterns to detect.
...
Scan backend/fullstack project(s) for route definitions. Read `references/detection-patterns.md` for framework-specific patterns.
...
Scan frontend project(s) for outgoing HTTP calls. Read `references/detection-patterns.md` for call patterns.
...
Produce a single Markdown file. Read `references/output-template.md` for the exact structure to follow.
